...
// -----------------------------------------------------------------------------
// PBRFBeamEncoder  (JIT-fused: no host-side kernels)
// -----------------------------------------------------------------------------

using namespace rfnn::tcnn;


PBRFBeamEncoder::PBRFBeamEncoder(uint32_t spectrum_dim, uint32_t d_model, uint32_t distance_bins)
    : spectrum_dim(spectrum_dim),
      d_model(d_model),
      distance_bins(distance_bins),
      NEXT_MULTIPLE_FOR_TYPE((sizeof(::tcnn::network_precision_t) == 2) ? 8 : 4)
{
    // 2-layer MLP for the spectrum. Choose an internal width that matches
    // d_model so the encoded spectrum slot in the ParameterSet has a balanced
    // contribution alongside direction (16) and distance (16).
    this->spectrum_enc_dim = d_model;
    this->spectrum_dim_padded = ::tcnn::next_multiple<uint32_t>(spectrum_dim, 16u);

    // No LayerNorm on the raw spectrum input — the input histogram already
    // has sum=1 and per-bin variance carries useful magnitude info.
    this->spectrum_mlp1.reset(
        ::tcnn::create_network<::tcnn::network_precision_t>(
            ::tcnn::json{
                ...
                {"n_hidden_layers", 1},          // 2 weight matrices = "2-layer MLP"
                // SiLU mirrors PBRFNet's `RFBackboneModel.spectra_encoder`
                // (nn.SiLU). ReLU's dead-neuron failure in fp16 — once an
                // output channel's pre-activation goes negative the gradient
                // is zero forever — is a plausible contributor to the CPP
                // val_spectrum_accuracy plateau (0.47 vs Python's 0.66).
                {"activation", "SiLU"},
    ...
    // Second LayerNorm + MLP pair on top: gives us "2-layer MLP with LayerNorm"
    // (LN between the two MLPs as well as before them).
    ...
    std::vector<ParameterSetEncoding::ParameterSet> sets = {
        ...
        ParameterSetEncoding::ParameterSet("one_blob",            1, this->distance_bins),
    ...
    // LayerNorm on the d_model-wide ParameterSetEncoding output (mirrors the
    // LayerNorm PBRFNet places after the final Linear in its beam encoder).
    this->beam_params_ln.reset(new rfnn::tcnn::LayerNorm(d_model));

    // Every sub-module is invoked through generate_*_device_function below,
    // so they must run on the JIT path.
...
PBRFBeamEncoder::~PBRFBeamEncoder() = default;

size_t PBRFBeamEncoder::n_params() const {
    size_t n = 0;
...
std::vector<std::pair<uint32_t, uint32_t>> PBRFBeamEncoder::layer_sizes() const {
...
void PBRFBeamEncoder::set_params_impl(::tcnn::network_precision_t* params, ::tcnn::network_precision_t* inference_params, ::tcnn::network_precision_t* gradients) {
    size_t offset = 0;
    auto bump = [&](size_t n) {
...
void PBRFBeamEncoder::initialize_params(::tcnn::pcg32& rnd, float* p, float scale) {
    float* cur = p;
    auto step = [&](auto& m) {
...
uint32_t PBRFBeamEncoder::device_function_fwd_ctx_bytes() const {
    uint32_t bytes = 0;
...
uint32_t PBRFBeamEncoder::backward_device_function_shmem_bytes(uint32_t n_threads, ::tcnn::GradientMode mode) const {
    uint32_t bytes = 0;
...
void PBRFBeamEncoder::convert_params_to_jit_layout(cudaStream_t stream, bool use_inference_params) {
    // LayerNorm and ParameterSetEncoding store params in JIT layout already;
    // only the CutlassMLPs need reordering.
...
void PBRFBeamEncoder::convert_params_from_jit_layout(cudaStream_t stream, bool use_inference_params) {
...
std::string PBRFBeamEncoder::generate_device_function(const std::string& fn_name) const {
    // No spectrum_ln1 — the first LayerNorm on the raw 32-bin (sum=1)
    // spectrum was a mistake (suspected cause of CPP val_spectrum_accuracy
    // plateau). Mirrors the Python SimpleSpectraEncoder.
    ...
    // Input layout (float, total = 4 + spectrum_dim):
    //   input[0..3)            direction (3)
    //   input[3]               distance
    //   input[4..4+spectrum_dim) spectrum
    // Spectrum is zero-padded to spectrum_dim_padded and fed directly to MLP1.
    std::string body = fmt::format(R"(
        ...
        TCNN_PRAGMA_UNROLL
        for (int i = 0; i < {SPEC}; i++) spec_in[i] = (::tcnn::network_precision_t)input[4 + i];
        ...
        // 5) Assemble [direction(3), distance(1), spectrum_encoded(d_model)] (float)
        ...
        concat[3] = input[3];
        TCNN_PRAGMA_UNROLL
        for (int i = 0; i < {D}; i++) concat[4 + i] = (float)mlp2_out[i];

        // 6) Final beam parameter encoding
        auto pse_out = {FN_NAME}_pse(concat, params + {PSE_P}, fwd_ctx ? fwd_ctx + WARP_SIZE * {PSE_O} : nullptr);

        // 7) LayerNorm on the d_model-wide ParameterSetEncoding output. The
        //    `beam_encoded` slice that downstream FiLM layers consume needs
        //    unit variance per sample; without this LN the ReLU trunk in
        //    BaseRadiationPredictionModel collapses to a constant flux
        ...
        fmt::arg("CONCAT_W", 4u + this->spectrum_enc_dim),
...
std::string PBRFBeamEncoder::generate_backward_device_function(const std::string& fn_name, uint32_t n_threads) const {
    const uint32_t MLP1_P   = 0;
    const uint32_t LN2_P    = MLP1_P   + ::tcnn::next_multiple<uint32_t>(this->spectrum_mlp1->n_params(),        NEXT_MULTIPLE_FOR_TYPE);
    const uint32_t MLP2_P   = LN2_P    + ::tcnn::next_multiple<uint32_t>(this->spectrum_ln2->n_params(),         NEXT_MULTIPLE_FOR_TYPE);
    const uint32_t PSE_P    = MLP2_P   + ::tcnn::next_multiple<uint32_t>(this->spectrum_mlp2->n_params(),        NEXT_MULTIPLE_FOR_TYPE);
    ...
    std::string body = fmt::format(R"(
        // 0) Out-LN backward: dL_dy (half) -> dL_dpse_f (float, D)
        ...
        // 2) Split dL_dconcat: rows [4..4+D) are dL/d(mlp2_out) (float, needs half cast)
        ::tcnn::tvec<::tcnn::network_precision_t, {D}> dL_dmlp2_out;
        TCNN_PRAGMA_UNROLL
        for (int i = 0; i < {D}; i++) dL_dmlp2_out[i] = (::tcnn::network_precision_t)dL_dconcat[4 + i];

        // 3) MLP2 backward -> dL_dln2_out (half)
        ...
        // 4) LN2 backward -> dL_dmlp1_out_float
        ...
        // 7) Assemble dL_dx: direction/distance from dL_dconcat[0..4],
        //    spectrum cast back to float at the encoder-input boundary.
        if (dL_dx) {{
            (*dL_dx)[0] = dL_dconcat[0];
            (*dL_dx)[1] = dL_dconcat[1];
            (*dL_dx)[2] = dL_dconcat[2];
            (*dL_dx)[3] = dL_dconcat[3];
            TCNN_PRAGMA_UNROLL
            for (int i = 0; i < {SPEC}; i++) (*dL_dx)[4 + i] = (float)dL_dspec_h[i];
        }}
    )",
        fmt::arg("FN_NAME",  fn_name),
        fmt::arg("SPEC",     this->spectrum_dim),
