from rftypes import AirKermaField, RadiationFieldChannel, TrainingInputData, RadiationField
...
class AirkermaScatterAccuracy(AirkermaAccuracy):
    ...
    def forward(self, target: Union[RadiationFieldChannel, AirKermaField, Tensor], prediction: Union[RadiationFieldChannel, AirKermaField, Tensor], input: TrainingInputData = None) -> Tensor:
        ...
        beam_mask = xgt > xgt.max() * self.max_relative_fluence  # ignore areas with > max_relative_fluence of max primary flux
        low_fluence_mask_gt = fgt < fgt.max() * self.min_relative_fluence  # ignore areas with < min_relative_fluence of max total flux
        if isinstance(prediction, RadiationFieldChannel):
            low_fluence_mask = prediction.flux < prediction.flux.max() * self.min_relative_fluence  # ignore areas with < min_relative_fluence of max predicted flux
        elif isinstance(prediction, AirKermaField):
