...
class FiLM(FusionBase):
    """
    FiLM (Feature-wise Linear Modulation) layer from the paper "FiLM: Visual Reasoning with a General Conditioning Layer" (2017).

    Fusion contract (see :class:`FusionBase`): ``forward(x, condition) -> y`` with dim(y)==dim(x);
    ``x`` is the modulated (location) feature, ``condition`` the (beam) conditioning vector.
    """
