...
class ValidateGroundTruth(Callback):
    ...
    def on_train_batch_start(self, trainer: Trainer, pl_module: BaseNeuralRadFieldModel, batch: TrainingInputData, batch_idx: int) -> None:
            ...
            if isinstance(gt, RadiationFieldChannel):
                ...
                zero_counts_map = gt.fluence < 1e-6
                zeros_sum_spectra = (spectrum_sums < 1e-6)
                same_places = (zeros_sum_spectra == zero_counts_map)
                assert torch.all(same_places), "Ground truth validation failed: mismatch between zero spectra sum and zero fluence places!"
                assert torch.isclose(spectrum_sums[~zeros_sum_spectra], 1.0, atol=1e-6), "Ground truth validation failed: non-normalized spectra found where fluence is non-zero!"
