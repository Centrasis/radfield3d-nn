...
from rftypes import TrainingInputData, DirectionalInput, PositionalInput
    ...
    def forward(self, x: TrainingInputData) -> TrainingInputData:
        ...
        with torch.no_grad():
            ...
            if FieldShape(int(x.input.beam_shape_type[0, 0].item())) == FieldShape.CONE and (x.input.beam_shape_type[:, 0] == float(int(FieldShape.CONE))).all():
                ...
                range_span = opening_angle_range[:, 1] - opening_angle_range[:, 0]
                if (range_span > 1e-8).all():
                    x.input.beam_shape_parameters[:, 0] = (torch.deg2rad(x.input.beam_shape_parameters[:, 0]) - opening_angle_range[:, 0]) / range_span  # normalize to [0, 1]
                else:
