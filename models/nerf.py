...
from rftypes import AirKermaField, RadiationField, RadiationFieldChannel, PositionalInput, DirectionalInput
...
from layers.film import ResidualFiLM, FiLM
from normalizations.linear import LinearNormalizer
from layers.gates import GatedFusion
...
class SRBFNet(RFNetBase):
    ...
    def __init__(self, location_encoding_dims=10, direction_encoding_dims=10, d_model=256, out_spectra_dim=32, flux_loss="L1LogLoss", spectrum_loss="HistogramLoss", learning_rate: float=1e-3, randomize_voxel_location_in_training: bool = True, voxels_centered_around_origin: bool = True, normalizer=None, conditioning: Literal["None", "FiLM", "ResFiLM", "AttentionConditioning", "Hypernetwork", "Gated"] = "None", flux_activation:str = None):
        ...
        self.fluence_loss_name = flux_loss
        self.spectrum_loss_name = spectrum_loss

        self._fluence_loss_fn = ModuleBuilder.ConstructLoss_fn(flux_loss)
        ...
        if flux_activation is not None:
            print("WARNING: For publication flux-activation was effectively used as sigmoid! Activation will be overriden!")
    ...
    def decode_results(self, model_output: Tensor) -> RadiationField | AirKermaField:
        ...
        flux = self.fluence_decoder(model_output).squeeze(-1)
    ...
    def configure_beam_encoding(self, conditioning: Literal["None", "FiLM", "ResFiLM", "AttentionConditioning", "Hypernetwork", "Gated"], beam_param_dims: int, d_model: int, beam_param_components_dims: list = None):
        ...
        elif conditioning == "AttentionConditioning":
            self.use_conditioning = True
            self.first_layer_xyz_only = True
            self.beam_conditioner1 = CrossAttentionConditioner(d_model, 32)
            self.beam_conditioner2 = CrossAttentionConditioner(d_model, 32)
    ...
    def get_custom_parameters(self):
        return {
            ...
            "flux_loss": self.fluence_loss_name,
...
class PBRFNet(SPERFNet):
    ...
    def __init__(self, location_encoding_dims=10, direction_encoding_dims=10, d_model=256, out_spectra_dim=32, in_spectra_dim=150, encoded_spectra_dims=64, scalar_encoding_dims=16, use_spectra_encoding=False, flux_loss="L1LogLoss", spectrum_loss="HistogramLoss", learning_rate = 0.001, randomize_voxel_location_in_training = True, voxels_centered_around_origin = True, conditioning: Literal["None", "FiLM", "ResFiLM", "AttentionConditioning", "Gated"] = "None", use_beam_shape: bool = True, normalizer=None, flux_activation:str = None):
        super().__init__(location_encoding_dims, direction_encoding_dims, d_model, out_spectra_dim, in_spectra_dim, encoded_spectra_dims, use_spectra_encoding, flux_loss, spectrum_loss, learning_rate, randomize_voxel_location_in_training, voxels_centered_around_origin, conditioning=conditioning, normalizer=normalizer, flux_activation=flux_activation)
