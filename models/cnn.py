...
from .base import BaseNeuralRadFieldModel, AngularSinusoidalFrequencyEncoding, SinusoidalFrequencyEncoding
from typing import Type
from rftypes import DirectionalInput, RadiationField, RadiationFieldChannel, TrainingInputData
...
class DeConvBase(BaseNeuralRadFieldModel):
    ...
    def forward2volume_from_training_input(self, batch: TrainingInputData, voxel_counts: Tensor, spectra_bins: int = 32) -> RadiationField:
        ...
        gt_fluence = (batch.ground_truth.scatter_field.flux + (batch.ground_truth.direct_beam.flux if batch.ground_truth.direct_beam is not None else 0.0) if isinstance(batch.ground_truth, RadiationField) else batch.ground_truth.flux)
    ...
    def forward2volume(self, x, voxel_counts: Tensor, spectra_bins: int = 32, mask: Tensor | None = None) -> RadiationField:
        ...
        if mask is not None:
            inf_fluence = torch.full_like(pred_field.scatter_field.flux, -torch.inf)
            ...
            pred_field = RadiationField(
                scatter_field=RadiationFieldChannel(
                    ...
                    flux=torch.where(mask, inf_fluence, pred_field.scatter_field.flux) if mask is not None else pred_field.scatter_field.flux,
                ...
                direct_beam=RadiationFieldChannel(
                    ...
                    flux=torch.where(mask, inf_fluence, pred_field.direct_beam.flux) if mask is not None else pred_field.direct_beam.flux,
...
class DeConvRadFieldV1(DeConvBase):
    __model_name__ = "DeConvRadFieldV1"

    def __init__(self, pos_enc_dim=4, d_model=256, spectra_bins: int = 150, in_spectra_bins: int = 64, encoded_spectra_bins: int = 32, out_spectra_bins: int = 32, out_dims: tuple[int, int, int] = (50, 50, 50), use_self_attention: bool = False):
        super().__init__(d_model=d_model, spectra_bins=spectra_bins, out_dims=out_dims)
        self.positional_direction_encoding = AngularSinusoidalFrequencyEncoding(pos_enc_dim, append_input=False)
        self.pos_enc_dim = pos_enc_dim
        self.out_spectra_bins = out_spectra_bins
        self.angle_encoder = nn.Sequential(
            nn.Linear(self.positional_direction_encoding.encoded_dims, d_model),
        ...
        self.encoded_spectra_bins = encoded_spectra_bins
        self.use_self_attention = use_self_attention
        self.spectra_encoder = nn.Sequential(
            nn.Linear(in_spectra_bins, encoded_spectra_bins),
            nn.LayerNorm(encoded_spectra_bins),
            nn.SiLU(),
        )
        self.spectra_self_attention = CoordinateAttention(self.encoded_spectra_bins) if use_self_attention else nn.Identity()
        self.input_encoder = nn.Sequential(
            nn.Linear(d_model + encoded_spectra_bins, d_model),
            nn.LayerNorm(d_model),
            nn.SiLU()
        )
        deconv_blocks = self.generate_deconvolution(out_spectra_bins + 1, out_dims)
        self.deconvolution = nn.Sequential(*[m for m in deconv_blocks])
        self.spectrum_activation = HistogramNormalize(dim=1)
    ...
    def forward(self, input: DirectionalInput) -> RadiationField:
        """
        @param input: TrainingInputData with input and ground truth
        """
        abc_enc = self.positional_direction_encoding(input.direction)
        spectra = resample_histogram_bilinear(input.spectrum, self.in_spectra_bins)
        spectra = self.spectra_encoder(spectra)
        spectra = self.spectra_self_attention(spectra)
        x1 = self.angle_encoder(abc_enc)
        x1_1 = torch.cat((x1, spectra), dim=-1)
        x2: Tensor = self.input_encoder(x1_1)
        x2 = x2.view(-1, self.d_model, 1, 1, 1)

        x3 = self.deconvolution(x2)
        spectra = self.spectrum_activation(x3[:, :self.out_spectra_bins, :self.out_dims[0], :self.out_dims[1], :self.out_dims[2]])
        flux = self.flux_activation(x3[:, -1, :self.out_dims[0], :self.out_dims[1], :self.out_dims[2]]).unsqueeze(1)
...
class Beam2ScatterUNet(DeConvBase):
    ...
    def forward(self, batch: TrainingInputData) -> RadiationField:
        ...
        padded_xray_fluence = nn.functional.pad(batch.ground_truth.direct_beam.flux, (
        ...
        for i, layer in enumerate(self.up_sampler):
            ...
            x: Tensor = layer(x)
            if self.use_spectra and i < len(self.up_sampler) - 1:    # only apply FiLM to all but last layer
        ...
        if self.out_spectra_bins > 0:
            out_spectra = self.spectra_activation(x[:, :self.out_spectra_bins, :, :, :])
        else:
            out_spectra = None

        return RadiationField(
            scatter_field=RadiationFieldChannel(
                spectrum=out_spectra,
                flux=flux,
                error=None
            ),
            direct_beam=None
