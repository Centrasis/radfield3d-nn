...
from encodings.sinusoidal_encoding import SinusoidalFrequencyEncoding, AngularSinusoidalFrequencyEncoding
from encodings.hash_encoding import HashGridEncoding
from encodings.spherical_hamonics import SphericalHarmonics
...
class BaseNeuralRadFieldModel(pl.LightningModule):
    ...
    def __init__(self, location_encoding_dims=10, direction_encoding_dims=4, d_model=256, normalizer: Union[Normalizer, str] = LinearNormalizer(), learning_rate: float=1e-3):
        ...
        self._spectrum_loss_fn: Loss = comb_loss.HistogramLoss(dim=1) # comb_loss.HistogramLoss(bin_dim=1, weight_with_error=False, penalize_out_of_range=False) # std.KLDivLossWeighted(True)
