...
from normalizations import Normalizer, LinearNormalizer, NormalizerConstructor
from models.encoders.sinusoidal_encoding import SinusoidalFrequencyEncoding
from rftypes import AirKermaField, RadiationField, PositionalInput, TrainingInputData, RadiationFieldChannel, DirectionalInput, PositionalInput
import gc
from rich import print
from typing import Union
from losses.base import Loss
from metrics.types import TrainingMetrics, ChannelMetrics
import losses.std as std
import losses.combinations as comb_loss
...
class ModuleBuilder:
    ...
    def ConstructLoss_fn(loss_fn_name: str) -> nn.Module:
        ...
        elif loss_fn_name == "VectorialL1Loss":
            return std.VectorialL1LossWeighted(dim=1, weight_with_error=False, reduction='mean')
        elif loss_fn_name == "VectorialL1LossLogSpace":
            return std.VectorialL1LossLogSpace(dim=1, weight_with_error=False, reduction='mean')
...
class BaseNeuralRadFieldModel(pl.LightningModule):
    ...
    def __init__(self, location_encoding_dims=10, direction_encoding_dims=4, d_model=256, normalizer: Union[Normalizer, str] = LinearNormalizer(), learning_rate: float=1e-3):
        ...
        self._spectrum_loss_fn: Loss = std.WassersteinLossWeighted(dim=1) # comb_loss.HistogramLoss(bin_dim=1, weight_with_error=False, penalize_out_of_range=False) # std.KLDivLossWeighted(True)
        self._geometry_loss_fn: Loss = std.FocalBCEWithLogitsWeighted(weight_with_error=False)
        #self._fluence_spectrum_loss_blend = nn.Parameter(torch.tensor(0.5), requires_grad=True) # Multi-Task Learning weight between flux and spectrum loss
         # 0.0 = only flux loss, 1.0 = only spectrum loss
         # will be optimized during training
         # from "Multi-Task Learning Using Uncertainty to Weigh Losses for Scene Geometry and Semantics" (2018, CVPR)
        ...
        self.save_hyperparameters(ignore=["indices", "grid_dims", "_fluence_loss_fn", "_spectrum_loss_fn", "normalizer", "_geometry_loss_fn", "_channels_join"])
    ...
    def on_fit_start(self):
        device = next(self.parameters()).device
        #self._fluence_spectrum_loss_blend = self._fluence_spectrum_loss_blend.to(device)
    ...
    def create_radiationfield_like(field: RadiationField, batch_size: int = None) -> RadiationField:
        ...
        fluence_field_shape = [batch_size] + list(field.scatter_field.flux.shape) if batch_size is not None else list(field.scatter_field.flux.shape)
        ...
        return RadiationField(
            scatter_field=RadiationFieldChannel(
                ...
                flux=torch.empty(fluence_field_shape, dtype=field.scatter_field.flux.dtype, device=field.scatter_field.flux.device) if field.scatter_field.flux is not None else None,
            ...
            direct_beam=RadiationFieldChannel(
                ...
                flux= torch.empty(fluence_field_shape, dtype=field.direct_beam.flux.dtype, device=field.direct_beam.flux.device) if field.direct_beam.flux is not None else None,
    ...
    def calculate_metrics(self, pred_field: RadiationField | AirKermaField, y: RadiationField | AirKermaField, batch: TrainingInputData, ignore_scatter: bool = False, ignore_xray_beam: bool = False) -> TrainingMetrics:
        ...
        if pred_field.geometry is not None:
            assert batch.input.geometry is not None, "Input geometry must be provided in the batch when predicting geometry."
            geom_loss = self._geometry_loss_fn.forward(prediction=pred_field.geometry, target=batch.input.geometry, input=batch)
        ...
        if isinstance(pred_field, RadiationField):
            ...
            if pred_field.direct_beam is not None and has_multi_channel and not ignore_xray_beam:
                loss_spec = None
                ...
                loss_fluence = self._fluence_loss_fn.forward(prediction=pred_field.direct_beam.flux, target=y.direct_beam.flux, input=batch)

                xray_metrics = ChannelMetrics(
                    flux_loss=loss_fluence,
            ...
            if pred_field.scatter_field is not None and not ignore_scatter:
                ...
                loss_fluence = self._fluence_loss_fn.forward(prediction=pred_field.scatter_field.flux, target=scatter_field_gt.flux, input=batch)
                ...
                if xray_metrics is not None and y.direct_beam is not None:
                    sum_fluence_ratio = y.direct_beam.flux.sum() / scatter_field_gt.flux.sum()
            ...
            return TrainingMetrics(
                ...
                direct_beam=xray_metrics,
                geometry=geom_loss if pred_field.geometry is not None else None
            )
    ...
    def process_metrics(self, metrics: TrainingMetrics, stage: str) -> Tensor:
        ...
        #self.log(f'{stage}_fluence_loss_weight', torch.sigmoid(self._fluence_spectrum_loss_blend), on_epoch=on_epoch, on_step=on_step, logger=True, batch_size=self.batch_size)
        if metrics.scatter_field is not None:
            ...
            if metrics.scatter_field.spectrum_loss is not None:
                #loss_weight = torch.sigmoid(self._fluence_spectrum_loss_blend)
                loss_weight = 0.5
            ...
            self.log(f'{stage}_scatter_fluence_loss', metrics.scatter_field.flux_loss.mean(), on_epoch=on_epoch, on_step=on_step, logger=True, batch_size=self.batch_size)
        ...
        if metrics.direct_beam is not None:
            if metrics.direct_beam.spectrum_loss is not None:
                ...
                self.log(f'{stage}_xray_beam_spectrum_loss', metrics.direct_beam.spectrum_loss.mean(), on_epoch=on_epoch, on_step=on_step, logger=True, batch_size=self.batch_size)
            else:
                total_loss = total_loss + metrics.direct_beam.flux_loss
            self.log(f'{stage}_xray_beam_fluence_loss', metrics.direct_beam.flux_loss.mean(), on_epoch=on_epoch, on_step=on_step, logger=True, batch_size=self.batch_size)
        ...
        if metrics.geometry is not None:
            total_loss = total_loss * (2/3) + metrics.geometry * (1/3)
            self.log(f'{stage}_geometry_loss', metrics.geometry.mean(), on_epoch=on_epoch, on_step=on_step, logger=True, batch_size=self.batch_size)
