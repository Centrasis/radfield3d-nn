...
from rftypes import AirKermaField, RadiationField, PositionalInput, RadiationFieldChannel, DirectionalInput, PositionalInput
from typing import Union
from activations.HistogramNormalize import HistogramNormalize
from utils.samplings.mean_sampling import resample_histogram_bilinear
...
class MultilayerDecoder(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, scales: int = 1):
        super().__init__()
        self.in_dim = in_dim
        ...
        for i in range(scales):
            self.blocks.append(nn.Sequential(
                nn.Linear(in_dim, in_dim // (i+2)),
                nn.LayerNorm(in_dim // (i+2)),
                nn.Dropout(0.1),
                nn.SiLU(),
                nn.Linear(in_dim // (i+2), out_dim),
                nn.LayerNorm(out_dim)
            ))
        self.fusion = nn.Linear(out_dim * scales, out_dim)

    def forward(self, x: Tensor) -> Tensor:
        temp = torch.empty((x.shape[0], self.out_dim * self.scales), device=x.device, dtype=x.dtype)
        for i in range(self.scales):
            temp[:, i * self.out_dim:(i + 1) * self.out_dim] = self.blocks[i](x)
        x = temp.view(-1, self.scales * self.out_dim)
        x = self.fusion(x)
        return x
...
class FeedforwardPointwiseModel(BaseNeuralRadFieldModel):
    ...
    def forward2volume(self, x: DirectionalInput, voxel_counts: Union[Tensor, torch.Size], spectra_bins: int = 32, mask: Tensor | None = None, global_parameters: Tensor | None = None) -> RadiationField | AirKermaField:
        ...
        if isinstance(full_field, RadiationField):
            full_field = RadiationField(
                scatter_field=RadiationFieldChannel(
                    flux=full_field.scatter_field.flux.view(*target_fluence_shape) if full_field.scatter_field.flux is not None else None,
                ...
                direct_beam=RadiationFieldChannel(
                    flux=full_field.direct_beam.flux.view(*target_fluence_shape) if full_field.direct_beam.flux is not None else None,
...
class ParametricFeedforwardModel(FeedforwardPointwiseModel):
    ...
    def __init__(self, location_encoding_dims=10, direction_encoding_dims=4, d_model=256, num_layers=5, out_spectra_dims: int = 32, activation_fn: str = "ReLU", spectra_loss_fn = "L1Loss", fluence_loss_fn = "L1Loss", position_encoding_type: str = "SinusoidalFrequency", layer_normalization: str | None = "LayerNorm", use_residuals: bool = True):
        ...
        self.fluence_loss_fn_name = fluence_loss_fn
        ...
        self.fluence_loss_fn = ModuleBuilder.ConstructLoss_fn(fluence_loss_fn)
        ...
        elif position_encoding_type == "DiscreteEmbeddingEncoding":
            self.preprocess_location = nn.Identity()
            self.preprocess_direction = lambda x: ((x + 1.0) / 2.0)
            self.positional_location_encoding = DiscreteEmbeddingEncoding(3, location_encoding_dims * 8, append_input=True)
            self.positional_direction_encoding = DiscreteEmbeddingEncoding(3, direction_encoding_dims * 8, append_input=False)
        else:
    ...
    def get_custom_parameters(self):
        return {
            ...
            "fluence_loss_fn": self.fluence_loss_fn_name,
...
class WangFCN(FeedforwardPointwiseModel):
    ...
    def forward(self, x: PositionalInput) -> RadiationField:
        ...
        flux = self.fluence_decoder(x)
        flux = self.fluence_activation_fn(flux)
...
class FlatFCNField(FeedforwardPointwiseModel):
    __model_name__ = "FlatFCNField"

    def __init__(self, location_encoding_dims=10, direction_encoding_dims=4, d_model=256, num_layers=5, out_spectra_dims: int = 32, in_spectra_bins: int = 64, encoded_spectra_bins: int = 32, position_encoding_type: str = "SinusoidalFrequency", use_spectra_encoding: bool = True, use_residuals: bool = True, use_spectra_self_attention: bool = False):
        super().__init__(location_encoding_dims, direction_encoding_dims, d_model)
        self.num_layers = num_layers
        self.out_spectra_dims = out_spectra_dims
        self.use_residuals = use_residuals
        ...
        self.position_encoding_type = position_encoding_type
        self.in_spectra_bins = in_spectra_bins
        self.encoded_spectra_bins = encoded_spectra_bins if use_spectra_encoding else in_spectra_bins
        self.use_spectra_encoding = use_spectra_encoding
        self.use_spectra_self_attention = use_spectra_self_attention
        self.positional_location_encoding = None
        self.positional_direction_encoding = None
        if self.position_encoding_type == "SinusoidalFrequency":
            self.preprocess_location = nn.Identity()
            self.preprocess_direction = nn.Identity()
            self.positional_location_encoding = SinusoidalFrequencyEncoding(location_encoding_dims, 3, append_input=True)
            self.positional_direction_encoding = AngularSinusoidalFrequencyEncoding(direction_encoding_dims, append_input=True)
        elif self.position_encoding_type == "CartesianDirectional":
            raise NotImplementedError("CartesianDirectionalEncoding is not implemented.")
            self.preprocess_location = nn.Identity()
            self.preprocess_direction = nn.Identity()
            # self.positional_location_encoding = 
            self.positional_direction_encoding = AngularSinusoidalFrequencyEncoding(direction_encoding_dims, append_input=True)
        elif self.position_encoding_type == "SphericalHarmonicsDirectional":
            self.preprocess_location = nn.Identity()
            self.preprocess_direction = nn.Identity()
            self.positional_location_encoding = SinusoidalFrequencyEncoding(location_encoding_dims, 3, append_input=True)
            self.positional_direction_encoding = AngularSinusoidalFrequencyEncoding(l_max=direction_encoding_dims // 2, append_input=True)
        else:
            raise ValueError(f"Unknown position encoding type: {position_encoding_type}")
        ...
        enc_blocks = [
            ResidualBlock(
                ...
                d_model=self.positional_location_encoding.encoded_dims + self.positional_direction_encoding.encoded_dims + self.encoded_spectra_bins,
                d_model_out=d_model,
                num_layers=1
            ) if use_residuals else LinearBlock(
                d_model=self.positional_location_encoding.encoded_dims + self.positional_direction_encoding.encoded_dims + self.encoded_spectra_bins,
                activation=nn.SiLU,
                normalization=nn.LayerNorm,
                num_layers=1,
        ...
        self.spectra_decoder = nn.Linear(d_model // 2, out_spectra_dims)
        self.spectra_encoder = nn.Sequential(
            nn.Linear(in_spectra_bins, self.encoded_spectra_bins),
            nn.LayerNorm(self.encoded_spectra_bins),
            nn.SiLU()
        ) if use_spectra_encoding else nn.Identity()
        self.spectra_self_attention = CrossCoordinateAttention(self.encoded_spectra_bins, self.encoded_spectra_bins) if use_spectra_self_attention else nn.Identity()
    ...
    def forward(self, x: PositionalInput) -> RadiationField:
        ...
        if x.spectrum is not None:
            spectrum = resample_histogram_bilinear(x.spectrum, self.in_spectra_bins)
            spectra_enc = self.spectra_encoder(spectrum)
        else:
            spectra_enc = torch.zeros(loc.shape[0], self.encoded_spectra_bins, device=loc.device, dtype=torch.float32)
        spectra_enc = self.spectra_self_attention(spectra_enc, spectra_enc)

        x = torch.cat((loc_enc, dir_enc, spectra_enc), dim=-1)
        x = self.encoding_block(x)

        flux = self.fluence_activation_fn(x[:, -1])
        spectra = self.spectra_decoder(x)
        spectra = self.spectra_activation_fn(spectra)
    ...
    def get_custom_parameters(self):
        return {
            ...
            "num_layers": self.num_layers,
            "out_spectra_dims": self.out_spectra_dims,
            "in_spectra_bins": self.in_spectra_bins,
            "encoded_spectra_bins": self.encoded_spectra_bins,
            "position_encoding_type": self.position_encoding_type,
            "use_spectra_encoding": self.use_spectra_encoding,
            "use_residuals": self.use_residuals,
            "use_spectra_self_attention": self.use_spectra_self_attention
        }
