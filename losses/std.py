...
import torch
from metrics.base import weight_field_by_statistical_error
...
class VectorialL1LossWeighted(StdLossWeighted):
    def __init__(self, dim: int = -1, weight_with_error: bool = False, reduction: str = 'mean'):
        super().__init__(nn.L1Loss(reduction='none'), weight_with_error)
        self.dim = dim
        self.reduction = reduction

    def forward(self, target: Tensor, prediction: Tensor, input: TrainingInputData) -> Tensor:
        assert len(prediction.shape) == len(target.shape), f"Prediction shape {prediction.shape} and target shape {target.shape} do not match."
        losses = torch.abs(target - prediction)
        losses = torch.nan_to_num(losses, nan=1.0, posinf=1.0, neginf=1.0)
        if self.reduction == 'sum':
            losses = torch.sum(losses, dim=self.dim).unsqueeze(self.dim)
        elif self.reduction == 'mean':
            losses = torch.mean(losses, dim=self.dim).unsqueeze(self.dim)
        elif self.reduction == 'none':
            losses = losses
        else:
...
class VectorialL1LossLogSpace(StdLossWeighted):
    def __init__(self, dim: int = -1, weight_with_error: bool = False, reduction: str = 'mean'):
        super().__init__(None, weight_with_error)
        self.dim = dim
        self.reduction = reduction
    ...
    def forward(self, target: Tensor, prediction: Tensor, input: TrainingInputData) -> Tensor:
        ...
        losses = self.l1_loss(target_log, prediction_log)
...
class JensenShannonLossWeighted(StdLossWeighted):
    def __init__(self, weight_with_error: bool = False, eps: float = 1e-8):
        super().__init__(None, weight_with_error)
        self.eps = torch.tensor(eps, dtype=torch.float32, requires_grad=False)
        self.kl_div = nn.KLDivLoss(log_target=True, reduction='none')

    def forward(self, target: Tensor, prediction: Tensor, input):
        if self.eps.device != target.device:
            self.eps = self.eps.to(target.device)
        target = torch.clamp(target.reshape(-1, target.shape[1]), min=self.eps)
        prediction = torch.clamp(prediction.reshape(-1, prediction.shape[1]), min=self.eps)
        m = torch.log(0.5 * (target + prediction))
        kl1 = self.kl_div(torch.log(target), m)
        kl2 = self.kl_div(torch.log(prediction), m)
        jsd = 0.5 * (kl1 + kl2)

        if self.weight_with_error:
            jsd = weight_field_by_statistical_error(jsd, input=input, eps=self.eps)

        return torch.mean(jsd)


class BCELossWeighted(StdLossWeighted):
    def __init__(self, weight_with_error: bool = False):
        super().__init__(nn.BCELoss(reduction='none'), weight_with_error)


class FocalBCEWithLogitsWeighted(StdLossWeighted):
    def __init__(self, alpha: float = 0.25, gamma: float = 2.0, weight_with_error: bool = False, reduction: str = 'mean', eps: float = 1e-8):
        super().__init__(None, weight_with_error)
        self.alpha = alpha
        self.gamma = gamma
        self.reduction = reduction
    ...
    def forward(self, target: Tensor, prediction: Tensor, input: TrainingInputData) -> Tensor:
        mask = torch.isinf(prediction) | torch.isnan(prediction)
        if mask.any():
            prediction = prediction.masked_fill(mask, 0.0)
        ...
        p_t = torch.clamp(target * p + (1.0 - target) * (1.0 - p), min=self.eps)
        alpha_t = target * self.alpha + (1.0 - target) * (1.0 - self.alpha)
        focal = alpha_t * (1.0 - p_t).pow(self.gamma)
        loss = focal * bce
        ...
        if self.reduction == "mean":
            ...
            if loss.dim() > 1:
                return torch.sum(loss, dim=tuple(range(1, loss.dim())))
            return torch.sum(loss)
        else:  # 'none'
            return loss


class FluenceTopKLogNCCLoss(StdLossWeighted):
    """
    NCC on the top-k% brightest target voxels (per sample), masked for non-finite values.
    Optionally applies log1p when inputs are non-negative to improve contrast near zero.
    """
    def __init__(self, weight_with_error: bool = False, k_frac: float = 0.2, alpha_l1: float = 0.65, eps: float = 1e-8, use_log1p: bool = True):
        super().__init__(loss_fn=None, weight_with_error=weight_with_error)
    ...
    def _maybe_log1p(self, t: Tensor, p: Tensor) -> tuple[Tensor, Tensor]:
        # Only apply log1p if tensors are non-negative (common for flux in physical units)
        if self.use_log1p and torch.all(t >= 0) and torch.all(p >= 0):
            return torch.log1p(t), torch.log1p(p)
        return t, p

    def _ncc_1d(self, a: Tensor, b: Tensor) -> Tensor:
        a = a - a.mean(dim=-1, keepdim=True)
        b = b - b.mean(dim=-1, keepdim=True)
        denom = (a.norm(dim=-1) * b.norm(dim=-1)).clamp_min(self.eps)
        ncc = (a.mul(b).sum(dim=-1) / denom).clamp(-1.0, 1.0)
        return ncc

    def forward(self, target: Tensor, prediction: Tensor, input: TrainingInputData) -> Tensor:
        # valid mask
        valid = torch.isfinite(target) & torch.isfinite(prediction)
        if valid.ndim < target.ndim:
            valid = valid.expand_as(target)

        t = torch.where(valid, target, torch.tensor(0.0, device=target.device, dtype=target.dtype))
        p = torch.where(valid, prediction, torch.tensor(0.0, device=prediction.device, dtype=prediction.dtype))

        # Collapse channel if (B,1,D,H,W) -> (B,D,H,W)
        if t.ndim >= 5 and t.size(1) == 1:
            t = t[:, 0]
            p = p[:, 0]
            valid = valid[:, 0]
        ...
        m_flat = valid.reshape(B, N)

        # Top-K per sample (use -inf for invalid so they never get selected)
        t_for_topk = t_flat.masked_fill(~m_flat, float("-inf"))
        K = max(2, int(self.k_frac * N))
        K = min(K, N)

        vals, idx = torch.topk(t_for_topk, k=K, dim=1, largest=True, sorted=False)
        # Gather selected elements
        t_sel = torch.gather(t_flat, 1, idx)
        p_sel = torch.gather(p_flat, 1, idx)
        m_sel = torch.gather(m_flat, 1, idx).float()  # 1 for valid, 0 for invalid among the top-k

        # Optional log1p to improve contrast near zero (safe: clamp to >=0 first)
        if self.use_log1p:
            t_sel = torch.log1p(torch.clamp(t_sel, min=0.0))
            p_sel = torch.log1p(torch.clamp(p_sel, min=0.0))

        # Masked per-sample mean
        count = m_sel.sum(dim=1).clamp_min(1.0)              # (B,)
        t_mean = (t_sel * m_sel).sum(dim=1, keepdim=True) / count.unsqueeze(1)
        p_mean = (p_sel * m_sel).sum(dim=1, keepdim=True) / count.unsqueeze(1)

        # Center and mask
        t_c = (t_sel - t_mean) * m_sel
        p_c = (p_sel - p_mean) * m_sel

        # NCC = sum(t_c * p_c) / (||t_c|| * ||p_c||)
        num = (t_c * p_c).sum(dim=1)                         # (B,)
        den = (t_c.pow(2).sum(dim=1).sqrt() * p_c.pow(2).sum(dim=1).sqrt()).clamp_min(self.eps)
        ncc = (num / den).clamp(-1.0, 1.0)                   # (B,)

        # Ignore samples with <2 valid voxels in Top-K
        valid_sample = (count >= 2).float()
        loss_per_sample = 1.0 - ncc
        loss = (loss_per_sample * valid_sample).sum() / valid_sample.sum().clamp_min(1.0)

        if self.weight_with_error:
            # Note: this applies a global weight; for voxelwise weights, move weighting before Top-K.
            loss = weight_field_by_statistical_error(loss, input=input)
